...
import { insertOrderSchema } from 'thermopolio-shared';
import { z } from 'zod';
...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    const plan = await storage.getSubscriptionPlan(orderData.planId);
    if (!plan) {
...
export const getOrders = async (req: Request, res: Response) => {
  try {
    ...
    if (req.session.userType === 'customer') {
      orders = await storage.getOrdersByUserId(req.session.userId);
    } else if (req.session.userType === 'tavola_calda') {
      orders = await storage.getOrdersByRestaurantId(req.session.userId);
    } else {
      return res.status(403).json({
        success: false,
    ...
    const enrichedOrders = await Promise.all(orders.map(async (order) => {
      ...
      if (req.session.userType === 'customer') {
        restaurant = await storage.getUser(order.restaurantId);
      } else {
